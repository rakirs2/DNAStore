...
{
    private Dictionary<DNASequence, int> _reverseComplements = new Dictionary<DNASequence, int>();
    private Dictionary<DNASequence, int> _inputs= new Dictionary<DNASequence, int>();


    public ReverseComplementDictionary()
    {
    }
    ...
    public void Add(DNASequence key)
    {
        ...
        if (!_inputs.TryAdd(key, 1))
        {
            _inputs[key] ++;
        }

        var rc = key.ToReverseComplement();
        if (!_reverseComplements.TryAdd(rc, 1))
        {
            _reverseComplements[rc] ++;
        }
    }

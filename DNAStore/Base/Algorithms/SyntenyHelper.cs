...
public static class SyntenyHelper
{
    ...
    public static IEnumerable<Tuple<int[], int, int>> AllPossibleReversals(int[] current, bool signed = false)
    {
        ...
        for (var j = i + 1; j < temp.Length; j++)
        {
            ...
            yield return new Tuple<int[], int, int>(seq, i, j);
            ReverseSubsequence(temp, i, j, signed);
        }
    }
    ...
    /// <param name="target"></param>
    /// <returns></returns>
    public static int MinimumalBreakPointReversals(int[] current, out List<Tuple<int[], int, int>> candidates,
        int[] target = null)
    {
        if (target == null)
        {
        ...
        foreach (var candidate in AllPossibleReversals(current))
        {
            ...
            if (numBreakPoints > reversalMax) continue;

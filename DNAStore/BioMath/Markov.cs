...
public class Markov
{
    ...
    public static double HiddenPathProbability(string pi, char[] states, double[,] transition)
    {
        ...
        for (var i = 1; i < pi.Length; i++) output *= transition[statesIndex[pi[i - 1]], statesIndex[pi[i]]];

        return output;
    }

    public static double PathOutcomeProbability(string outcome, char[] sigma, string pi, char[] states,
        double[,] emission)
    {
        var output = 1.0;
        ...
        for (var i = 0; i < outcome.Length; i++) output *= emission[statesIndex[pi[i]], alphabetIndex[outcome[i]]];
